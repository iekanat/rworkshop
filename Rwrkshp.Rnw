...
Classifiers: caret\footnote{Caret package is a wrapper for a host of classification and regression model training functions. It eases visualizations, data manipulation, and analytics among others.}, and class packages \vfill
...
<<'bar0',fig.width=6,fig.height=2.8,out.width='.7\\linewidth'>>=
# Load the Dataset
data(mtcars)
## ggplot2 package is already loaded, else library(ggplot2)

## Simple plot of just the means
...
<<'bar1',fig.width=6,fig.height=4,out.width='.7\\linewidth'>>=
...
# Instruct points to be plotted
	geom_point()
@

<<'scatter2', fig.width=6, fig.height=4, out.width='.7\\linewidth'>>=
ggplot(mtcars, aes(x=hp, y=mpg)) +
# Put some color in those points
...
# Let us create a simulated dataset to plot out categorical age
...
## Plot the results, with categorical age as lines o.
